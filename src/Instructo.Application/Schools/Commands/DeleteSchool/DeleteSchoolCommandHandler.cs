...
public class DeleteSchoolCommandHandler(
    ...
    IQueryRepository<School, SchoolId> schoolQueryRepository,
    ...
    ILogger<DeleteSchoolCommandHandler> logger) : ICommandHandler<DeleteSchoolCommand, Result<Unit>>
{
    public async Task<Result<Unit>> Handle(DeleteSchoolCommand request, CancellationToken cancellationToken)
    {
        ...
        var user = await userQueryRepository.GetUsersByIdBySuperAsync(userId);
        if(user is null)
        {
        ...
        {
            logger.LogError("Non authorized user {User} attempted to delete a {School}", new { User = user.Id }, new { School = existingSchool.Id });
            return Result<Unit>.WithErrors([new Error("Unauthorized", $"The user {userId} is not the owner of the school {existingSchool.Id}")]);
        }
